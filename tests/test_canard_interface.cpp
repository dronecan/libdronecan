...
class TestClient0 {
    CubeFramework::CanardTestInterface &interface;
public:
    TestClient0(CubeFramework::CanardTestInterface &_interface) : interface(_interface) {
    }
    ...
    CF_CREATE_CLIENT_CLASS(interface, // interface name
...
class TestClient1 {
    CubeFramework::CanardTestInterface &interface;
public:
    TestClient1(CubeFramework::CanardTestInterface &_interface) : interface(_interface) {
    }
    static int call_counts;
    void handle_get_node_info_response(const CanardRxTransfer &transfer, const uavcan_protocol_GetNodeInfoResponse &res) {
        ASSERT_EQ(res.status.uptime_sec, 1);
    ...
    CubeFramework::ObjCallback<TestClient1, uavcan_protocol_GetNodeInfoResponse> get_node_info_client_callback{this, &TestClient1::handle_get_node_info_response};
    CubeFramework::Client<uavcan_protocol_GetNodeInfo_cxx_iface, CubeFramework::HandlerList<1>> get_node_info_client{interface, get_node_info_client_callback};
};
...
class TestServer0 {
    CubeFramework::CanardTestInterface &interface;
public:
    TestServer0(CubeFramework::CanardTestInterface &_interface) : interface(_interface) {
    }
    static int call_counts;
    ...
    CF_CREATE_SERVER_CLASS(interface, // interface name
...
class TestServer1 {
    CubeFramework::CanardTestInterface &interface;
    CF_CREATE_SERVER_CLASS_INDEX(interface, // interface name
                                  1, // node index
                                  get_node_info_server, // server name
                                  uavcan_protocol_GetNodeInfo, // service name
...
public:
    TestServer1(CubeFramework::CanardTestInterface &_interface) : interface(_interface) {
    }
    static int call_counts;
    void handle_get_node_info_request(const CanardRxTransfer &transfer, const uavcan_protocol_GetNodeInfoRequest &req) {
        call_counts++;
...
TEST(StaticCanardTest, test_multiple_clients) {
    ...
    TestClient0 client0[2] = {test_interface_0, test_interface_0};
    TestClient1 client1[2] = {test_interface_1, test_interface_1};
    // create servers
    ...
    CF_CANARD_TEST_INTERFACE(0).update_tx(timestamp);
    CF_CANARD_TEST_INTERFACE(1).update_tx(timestamp);
